...
async function banir (client, from, chat, message, author, isGroupMsg) {
	...
	if(isGroupMsg) {
	    if(args.length >=2) {
            ...
            if (admins.includes(author) == true) {
		        ...
		        await client.reply(from, 'Somente admins pode usar esse comando', message)
	        }
	    }
    }           
}

async function promover (client, from, chat, message, author, isGroupMsg) {
    var admins = await client.getGroupAdmins(chat.id)
   if(isGroupMsg) {
       if(args.length >=2) {
            const wong = args[1]
...
async function rebaixar(client, from, chat, message, author, isGroupMsg) {
    var admins = await client.getGroupAdmins(chat.id)
	if(isGroupMsg){
		if(args.length >=2){
			const wong = args[1]
...
async function seticon (client, from, chat, message, author, isGroupMsg, quotedMsg) {
    ...
    if (isGroupMsg) {
        if (admins.includes(author) == true) {
            ...
            await client.reply(from, 'somente adms podem usar esse comando', message)
        }
    } else {
        client.sendText(from, 'Deve ser uma mensagem em grupo')
    }
}
