...
exports.mp3 = async function (message) {
	const path = './media/video/' + message.from + '.mp3';
  ...
  console.log('youtube-dl --extract-audio --audio-quality 0 --audio-format mp3  --output ' + `'./media/video/'${message.from}.%(ext)s" `  + link)
  await nrc.run('youtube-dl --extract-audio --audio-quality 0 --audio-format mp3  --output ' + `'./media/video/'${message.from}.%(ext)s" ` + link)
  if (Math.round(fs.statSync('./media/video/' + message.from + '.mp3').size / 1000000) > 99) {
  ...
  await gclient.sendPtt(from, './media/video/' + message.from + '.mp3', id)
  //await gclient.sendFile(message.from, './media/audio/' + message.from + '.mp3', '', '')
  if (Math.round(fs.statSync('./media/video/' + message.from + '.mp3').size / 1000000) == 0) {
    var größe = Math.round(fs.statSync('./media/video/' + message.from + '.mp3').size / 1000) + ' kB'
  } else {
    var größe = Math.round(fs.statSync('./media/video/' + message.from + '.mp3').size / 1000000) + ' MB'
  }
